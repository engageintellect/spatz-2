...
<div class="mt-5">
	...
	<div class="flex w-full gap-2 rounded-lg duration-300">
		...
		<textarea
			class="animate-textarea w-full resize-none rounded-lg bg-background text-lg focus-within:outline-none"
			{placeholder}
			{required}
			{disabled}
			{id}
		...
		<div class="item-center flex justify-between gap-5 text-sm">
			<Button
				type="submit"
	...
	<div
		class={`${value.length > 0 ? 'visible' : 'invisible'} ${value.length > 250 ? 'text-destructive' : ''} flex w-full`}
	>
		{value.length}/250
	</div>
</div>
