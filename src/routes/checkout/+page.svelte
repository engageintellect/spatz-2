		...
		<div class="w-full max-w-xs">
			<div class="contact-header flex flex-col items-start gap-2">
				<div class="flex w-full items-center justify-center gap-5">
					<div class="text-5xl font-bold lowercase">checkout</div>
					<Icon icon="material-symbols:shopping-cart-outline" class="contact-title-icon text-5xl" />
				</div>
			</div>
		</div>
	</div>
		...
		>
			<Form.Field {form} name="name">
				<Form.Control let:attrs>
					...
					<Input {...attrs} placeholder={$currentUser.name} bind:value={$formData.name} />
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
			...
			<Form.Field {form} name="email">
				<Form.Control let:attrs>
					...
					<Input {...attrs} placeholder={$currentUser.email} bind:value={$formData.email} />
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
			...
			<div class="mt-5">
				...
				<Form.Button disabled={isSubmitting} size="lg" class="group/sendButton mt-2 w-full">
					<div class="flex items-center gap-2">
						<div class="lowercase">checkout</div>
						<Icon
